...
const { cleanWord } = require("./cleanWord");
const { updateRateLimits } = require("./throttle");

const fetchAudioFromWordnik = async (word) => {
  word = cleanWord(word);
  const url = `https://api.wordnik.com/v4/word.json/${word}/audio?useCanonical=false&limit=10&api_key=${process.env.API_KEY}`;
  try {
    ...
    if (error.response && error.response.status === 429) {
      ...
      console.log(
        `Rate limit exceeded, retrying after ${retryAfter} milliseconds`,
      );
